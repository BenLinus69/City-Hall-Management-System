...
public class EntitateJuridica extends Utilizator{
    ...
    public String scriereCerere(String tipCerere,String fisierOut) throws Exception {
        String aux=TipCerere.creare_act_constitutiv.name();
        String auxf=aux.replace("_"," ");
        String aux2=TipCerere.reinnoire_autorizatie.name();
        String aux2f=aux2.replace("_"," ");

        if((tipCerere.compareTo(auxf))==0 )
        {
            return "Subsemnatul " + this.reprezentant + ", reprezentant legal al companiei "+ this.nume+ ", va rog sa-mi aprobati urmatoarea solicitare: "+ auxf;
        }
        else if((tipCerere.compareTo(aux2f))==0)
        {
            return "Subsemnatul " + this.reprezentant + ", reprezentant legal al companiei "+ this.nume+ ", va rog sa-mi aprobati urmatoarea solicitare: " +aux2f;
        }
            ...
            try{
                ...
                out.println("Utilizatorul de tip entitate juridica nu poate inainta o cerere de tip "+ tipCerere);
                bw.close();
            }
            catch (Exception e){
